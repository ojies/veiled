...
//!
//! Run: cargo run --example pedersen -p veiled-core
...
use veiled_core::{BlindingKey, Nullifier, commit};
use veiled_core::commitment::h_generator;

fn scalar(bytes: &[u8; 32]) -> Scalar {
    Scalar::reduce(U256::from_be_slice(bytes))
}

fn decode(c: &veiled_core::Commitment) -> ProjectivePoint {
