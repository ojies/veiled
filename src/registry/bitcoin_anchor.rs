...
use crate::core::{AnonymitySet, Commitment};
use crate::vtxo_tree::tree::build_tree;
use crate::vtxo_tree::types::{TreeNode, User};
...
mod tests {
    use super::*;
    use crate::core::{AnonymitySet, BlindingKey, Commitment, MasterSecret, Name, Nullifier};

    /// Create a valid commitment (actual secp256k1 point) using the core module.
    fn make_valid_commitment(seed: u8) -> Commitment {
        ...
        crate::core::commit(&nullifier, &blinding)
    }
    ...
    fn anchor_with_crs_commitments() {
        ...
        use crate::core::{Crs, User as VeiledUser};
        use crate::core::nullifier_v2::derive_all_nullifiers;

        let providers: Vec<VeiledUser> = (0..2)
            .map(|i| VeiledUser {
        ...
        for seed in 1..=4u8 {
            ...
            let name_scalar = crate::core::FriendlyName::new(format!("user-{seed}")).to_scalar_bytes();
